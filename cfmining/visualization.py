...
def buildTable(
    enumerator, pivot, criteria, names=None, include_original=True, include_cost=True
):
    ...
    eita = np.array([criteria.f(act) for act in enumerator.solutions])

    sort_idx = np.argsort([criteria.f(act) for act in enumerator.solutions])
...
class FeatureImportance:
    ...
    def calc(self):
        ...
        for nsol, sol in enumerate(self.enum.solutions):
            ...
            for idx, feat in enumerate(self.enum.names):
                ...
                if (self.enum.feas_grid[feat][-1] - self.enum.feas_grid[feat][0]) != 0:
                    cost_table[idx, nsol] = (
                        (sol[idx] - self.enum.feas_grid[feat][0])
                        / (self.enum.feas_grid[feat][-1] - self.enum.feas_grid[feat][0])
                        * changes
                    )
                else:
        ...
        with warnings.catch_warnings():
            ...
            change_level = pd.Series(
                np.nanmean(cost_table, axis=1), index=self.enum.names
            )
...
class PlotCounterfactuals:
    ...
    def __init__(
        self, actions, individual=None, apx=2, exclude_nanrows=True, height=None
    ):
        self.boxes = []
        if exclude_nanrows:
            ...
            if individual is not None:
                self.actions.insert(
                    0, "Original", individual[changes != 0][new_order], True
                )
        else:
        ...
        if height is None:
            self.height = [
                (self.H - i - 1, a) for i, a in enumerate(self.actions.index)
            ]
        else:
            self.height = height
        self.apx = apx
    ...
    def show(self, path=None, hl_column=[], hl_row=[], bbox_to_anchor=(-0.01, 0)):
        ...
        for i, feat in self.height:
            x = [
                value if ~np.isnan(ind) else np.nan
                for value, ind in zip(np.arange(self.W), self.actions.loc[feat].values)
            ]
            ...
            for xi in x:
                if ~np.isnan(xi):
                    self.ax.text(
                        xi,
                        i,
                        self.value(self.actions.loc[feat].values[xi]),
                        ha="center",
                        va="center",
                        fontsize=8.5,
                    )
        ...
        l = mlines.Line2D(
            [0.5, 0.5], [-0.5, self.H - 0.5], color="black", linewidth="1"
        )
        self.ax.add_line(l)
        rect = Rectangle(
            (-0.5, -0.5), 1, self.H, linewidth=0, edgecolor="k", facecolor="gainsboro"
        )
        ...
        plt.xticks(
            np.arange(self.W), ["Orig"] + ["C" + str(i) for i in range(1, self.W)]
        )
        plt.yticks([], [])
